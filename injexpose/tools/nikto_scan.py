...
def run_nikto_scan(
    ...
    if shutil.which("nikto") is None:
        raise NiktoError(
            "Nikto is not installed or not in PATH. Install with: sudo apt install nikto"
        )
    ...
    # Nikto sometimes returns non-zero even when it produced output.
    # Treat it as an error only if no report was created.
    ...
    # Append captured stdout/stderr at the bottom for debugging context
